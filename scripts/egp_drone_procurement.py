...
_i = KEEP.index("ชื่อผู้ชนะ") + 1
KEEP_OUT = (KEEP[:_i]
            + ["เลขนิติบุคคล_ใช้ได้", "_ผู้ชนะ_ปรับชื่อ", "_ตัดออกเพราะ", "_ชนิดงาน"]
            + KEEP[_i:])
...
# ⚠️ กติกาตัดต้อง **ผูกกับหมวด** และ **เข้าเงื่อนไขครบทุกกลุ่ม** ห้ามใช้คำเดี่ยวลอย ๆ
#    เคยใช้ ["ทางดิ่ง","แนวดิ่ง","กระโดดร่ม"] แบบไม่ผูกหมวด แล้วมันไปตัด
#    **โดรน VTOL 28 โครงการ (62 ล้าน)** ทิ้ง เพราะ "ขึ้น-ลงทางดิ่ง" คือคำแปลไทย
#    ของ Vertical Take-Off and Landing ซึ่งเป็นประเภทเดียวกับที่ ICS ผลิตขาย
#    → บทเรียน: คำที่ดูเฉพาะเจาะจงในบริบทหนึ่ง อาจเป็นคำสามัญในอีกบริบท
#
# โครงสร้าง: (เหตุผล, หมวดที่ใช้กติกานี้, [กลุ่มคำ AND กัน — ในกลุ่มเดียวกัน OR])
EXCLUDE_RULES = [
...
def prepare(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["_ตัดออกเพราะ"] = ""
    for reason, kinds, groups in EXCLUDE_RULES:
        hit = df["_หมวด"].isin(kinds) & (df["_ตัดออกเพราะ"] == "")
        for grp in groups:          # ทุกกลุ่มต้องเจอ (AND) ในกลุ่มเจออันใดอันหนึ่งก็ได้ (OR)
            hit &= low.str.contains("|".join(grp), na=False)
