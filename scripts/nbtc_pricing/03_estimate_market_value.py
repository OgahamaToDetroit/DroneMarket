...
prices = prices.rename(columns={"model": "model_final"}).merge(
    tiers[["brand", "model_final", "tier", "gen"]], on=["brand", "model_final"], how="left"
)
...
#   ชุดพร้อมบิน (โดรน+แบต+แท่นชาร์จ) : R² = 0.92  ราคาขึ้นกับความจุจริง
#   ตัวเครื่องเปล่า (โดรน+รีโมต)      : R² = 0.005 แทบไม่ขึ้นกับความจุเลย
...
fitsrc = allp[allp["L"].notna() & (allp["basis_kind"] == FIT_BASIS) & (allp["brand"] == "DJI")]
...
p(f"{'รวม':<10}{int(up['units'].sum()):>18,}{bt(up['val_lo'].sum()):>20}{bt(up['val_hi'].sum()):>20}")
p()
...
p("  วิธีนี้นับลำที่จดทะเบียนแล้วคูณราคาป้าย) แต่ได้ตัวเลขห่างกันไม่ถึง 10%")
p("  → เป็นการยืนยันข้ามแหล่งที่มีน้ำหนัก ไม่ใช่แค่ 'ไม่ขัดกัน'")
p()
