...
⛔ **ตัวเลขสัญญาภาครัฐในบล็อกถัดไปตายแล้ว — ต่ำไป ใช้อ้างอิงไม่ได้** (3 ส.ค. 2569)
   สคริปต์นี้นับจาก `egp_drone_projects.csv` ซึ่ง**กรองคีย์เวิร์ดโดรนมาก่อนแล้ว**
   งานของ ICS ที่ชื่อโครงการไม่มีคำว่าโดรนจึงหลุด พอค้นใหม่จาก **ชื่อผู้ชนะ** ตรง ๆ
   ได้ **5 สัญญา · 10,895,585 บาท** (ไม่ใช่ 3 สัญญา · 10.4 ล้าน)
   ที่เพิ่มมา: ปีงบ 2564 มทส. 378,790 บาท · ปีงบ 2568 มจธ. 136,809 บาท
   → ใช้ `scripts/ics_company_profile.py` แทน · ส่วนอื่นของสคริปต์นี้ยังใช้ได้
...
def main() -> None:
  ...
  4. เลขนิติบุคคลของปีงบ 2565-2567 ถูก Excel ทำพัง (1.05552E+11) เชื่อมกับ DBD
     ...
     (แก้แล้ว: เลขที่พังยังใช้ค้น datastore ได้ตรงตัว และเลขจริงของ ICS คือ
      0305560001659 ยืนยันจากแถวปีงบ 2568 — ดู ics_company_profile.py)
  5. ข้อมูลธุรกิจของ ICS มาจากเว็บบริษัทเอง ยังไม่ได้ยืนยันกับแหล่งอิสระ
  6. ⛔ ยอด 'ชนะ 3 สัญญา' ในหัวข้อ 2 ต่ำไป — นับจากชุดที่กรองคีย์เวิร์ดโดรนมาก่อน
     ยอดที่ครบคือ 5 สัญญา 10,895,585 บาท ดู scripts/ics_company_profile.py""")
